...
const base='https://hqsccc.pages.dev';
const site='花青素的主页';
...
const records=JSON.parse(fs.readFileSync(path.join(root,'content/articles.json'),'utf8'));
const slugs=new Set();
...
fs.cpSync(path.join(root,'vendor/katex/dist'),path.join(dist,'assets/katex'),{recursive:true});
fs.copyFileSync(path.join(root,'vendor/katex/LICENSE'),path.join(dist,'assets/katex/LICENSE'));
...
${home?'<meta name="google-site-verification" content="9mUTFcZFFlB0mivtvZEG3q-S_cQPp3TZMd6-afdgMgQ" />':''}
...
for(const a of records){
 const md=fs.readFileSync(path.join(root,'content',a.slug+'.md'),'utf8');
 let body=compile(md);
...
console.log('已生成 dist。请将其中全部文件上传到 Cloudflare Pages。');
